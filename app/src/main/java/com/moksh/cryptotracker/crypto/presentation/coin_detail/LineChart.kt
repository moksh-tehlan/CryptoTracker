...
fun LineChart(
    ...
    val visibleDataPoint = remember(dataPoint, visibleDataPointsIndices) {
        dataPoint.slice(visibleDataPointsIndices)
    }
    ...
    val selectedDataPointIndex = remember(dataPoint) {
    ...
    Canvas(
            ...
            .pointerInput(drawPoints, xLabelWidth) {
                detectHorizontalDragGestures { change, _ ->
                    ...
                    if(isShowingDataPoints){
                        onSelectedDataPoint(dataPoint[newSelectedDataPointIndex])
                    }
                }
            }
    ) {
        ...
        val xLabelTextLayoutResult = visibleDataPoint.map {
            measurer.measure(
                text = it.xLabel,
        ...
        val viewPortHeightPx = size.height -
                (verticalPaddingPx * 2 + xLabelLineHeight + maxXLabelHeight + xAxisLabelSpacingPx)
        ...
        val valueLabels = (0..labelCountExcludingLastLabel).map {
            ValueLabel(
        ...
        val yLabelTextLayoutResult = valueLabels.map {
            measurer.measure(
                text = it.formatted(),
        ...
        val remainingHeightForLabels = labelViewPortHeightPx - heightRequiredForYLabels
        ...
        drawPath(
            path = linePath, color = style.selectedColor, style = Stroke(
                width = 5f,
                cap = StrokeCap.Round
            )
        )
...
fun LineChartPreview() {
    CryptoTrackerTheme {
        val coinHistoryRandomized = remember {
            (1..120).map {
                CoinPrice(
                    priceUsd = ((1..100).random() * 1000).toDouble(),
